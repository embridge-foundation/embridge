...
const RELEASE_FILES = {
  rootSpec: path.join(root, 'embridge_format_specifications_v0.2.0.md'),
  archivedSpec: path.join(root, 'versions/v0_2_0/embridge_format_specifications_v0.2.0.md'),
  rootDemo: path.join(root, 'embridge_output_demo_v0.2.0.md'),
  archivedDemo: path.join(root, 'versions/v0_2_0/embridge_output_demo_v0.2.0.md'),
...
function runReleaseConsistencyChecks() {
  assert.strictEqual(
    ...
    'root v0.2.0 spec and archived v0.2.0 spec drift outside the documented boundary-comment clarification',
  );

  assert.strictEqual(
    read(RELEASE_FILES.rootDemo),
    read(RELEASE_FILES.archivedDemo),
    'root v0.2.0 demo and archived v0.2.0 demo must match',
  );
